...
set KBDest=c:\temp\Windows10-x64\KB_%COMPUTERNAME%_%sdate%-%stime%
set KBlog=c:\temp\Windows10-x64\Log_%COMPUTERNAME%_%sdate%-%stime%
...
echo.>>"%logfile%"
echo [%ldate% %ltime%] *** Suppression des KB localement
echo [%ldate% %ltime%] *** Suppression des KB localement >>"%logfile%"
echo [%ldate% %ltime%] *** Suppression des KB localement >>"%FileMonitorEncours%"
echo [%ldate% %ltime%] Commande : rd /Q /S "%KBDest%"  >>"%logfile%"
rd /Q /S "%KBDest%" >>"%logfile%" 2>&1
set err=%ERRORLEVEL%
...
if "%Context%". EQU "". echo [%ldate% %ltime%] *** Fin Script "%~n0"
if "%Context%". EQU "". echo [%ldate% %ltime%] *** Fin Script "%~n0" >>"%logfile%"
if "%Context%". EQU "". echo [%ldate% %ltime%] *** Fin Script "%~n0" >>"%FileMonitorEncours%"
if "%Context%". EQU "". type "%FileMonitorEncours%" >"%FileMonitorFini%"
if "%Context%". EQU "". del "%FileMonitorEncours%"

if "%Context%". EQU "". pause
if "%Context%". EQU "". exit /B 0
